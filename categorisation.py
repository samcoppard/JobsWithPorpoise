...
# Pull in the csv generated by scraping all the careers pages
scraped_csv = pd.read_csv('scraped_jobs.csv')

# Convert this variable into a dataframe
scraped_jobs = pd.DataFrame(scraped_csv)

# Remove all rows that have a missing value (whether that's job title, job description URL, or location)
...
bizdev = [
    ...
    'Associate Director', 'Partner Enablement', 'Partnerships Manager',
...
# Create a new dataframe that has a single row for each of the orgs with jobs, and a column that shows all the job types associated with all the jobs at that company
orgs = scraped_jobs.groupby('Company')['job_types'].apply(list).reset_index(
    name='job_types')
