...
from categorisation import map_jobs

@pytest.fixture
def sample_dataframe():
    """ Create a sample dataframe for testing """
    data = {
        "job_role": ['Junior Backend Software Engineer', 'Data Scientist', 'Woodland Creation Project Manager', 'Unexpected Job Role'],
...
def test_map_jobs_job_types(sample_dataframe):
    ...
    # Call the map_jobs function
    map_jobs(sample_dataframe, 'job_role', 'mapped_job_types', job_types_dict)

    # Verify the expected mappings
    assert sorted(sample_dataframe.loc[0, 'mapped_job_types'].split(", ")) == ['🤖 Software']
    assert sorted(sample_dataframe.loc[1, 'mapped_job_types'].split(", ")) == ['📊 Data & Analysis', '🤖 Software', '🧪 Science']
    assert sorted(sample_dataframe.loc[2, 'mapped_job_types'].split(", ")) == ['🌳 Rewilding', '🎯 Project Management', '🐝 Conservation']
...
def test_map_jobs_locations(sample_dataframe):
    ...
    assert sorted(sample_dataframe.loc[2, 'mapped_locations'].split(", ")) == ['North West', 'West Midlands']
