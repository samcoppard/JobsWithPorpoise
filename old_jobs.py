...
for row in rows:
    if row[0] not in scraped_jobs['concat'].tolist():
       ...
       # To use variables in the SQL query that are defined elsewhere in the script, we have to use parameterized queries
       cursor.execute("UPDATE jobs \
                      SET date_removed = TO_DATE(%s, 'YYYY-MM-DD') \
                      WHERE concat_name = %s \
                      AND date_removed IS NULL;", (str(date.today()), row[0]))
